---
name: done
description: Marks a task as completed and immediately shows what's next — closes the loop of "show me the next step, I did it, mark it, show me the one after that." Use whenever the user says they finished, completed, or did the current or a specific task — phrases like "marca como feito", "terminei essa tarefa", "concluí isso", "mark this as done", "I finished this", "done with this one", "marca a #20 como feita". Also triggers on explicit /done invocation, with or without a task ID. Do NOT use this to mark a task as started/in_progress (that's plain conversation, not this skill) or to just look at what's next without finishing anything (that's "next").
---
...
**If a task ID was given** (explicitly, like "/done 20" or "marca a #20 como feita"): that's the target. Confirm it exists via `TaskList` — if it doesn't, say so in one line and stop; don't touch anything.
...
If `TaskList` returns no tasks at all, or every task is already `completed`, there's nothing to mark — say the same thing `/next` would in that state ("nenhuma tarefa registrada" or "🎉 tudo concluído") and stop. This isn't an error; it's just nothing to do.
...
Mark straight to `completed` — never route through `in_progress` here, even for a task that was still `todo` or `blocked`. `in_progress` is for signaling "I'm working on this," which matters for longer or multi-session work; that stays a plain-language request outside this skill (e.g. "começando a #17"). A task this skill marks is, by definition, one the user is telling you is *finished* — for an atomic action like "tomar café," there was never a meaningful in-between state to pass through anyway. This also means a `blocked` task can be marked done directly if the user says so: the tracked blocker reflects the plan, not necessarily what happened in reality, and second-guessing that isn't this skill's job.

## Step 3: Show what's next

Immediately run `/next`'s full Steps 1-4 (selection + presentation) again and show the result, exactly as `/next` would render it — same table-free single-item format, same epic mention. Don't add a preamble like "marcado, agora vamos ver o próximo" — under the same terse spirit as `/progress` and `/next`, just show the confirmation and the next step, nothing narrated in between.
...
```
✅ #2 Implementar validação de webhook do Stripe — marcada como concluída.

epic: auth-service
#5 Implementar endpoint de reembolso — livre pra começar
```

**With ID, already completed:**
```
#15 já estava concluída.

epic: cronograma-mudanca
#16 Avisar síndico da data da mudança — livre pra começar
```

**Nothing left:**
```
✅ #22 Comer lanche da noite às 21h — marcada como concluída.
